...
class Collection(object):
	...
	""" Define collection.property as the union of all member.property values. """
...
class Resource(object):
	...
	return \
	    ...
	    Resource.m4_define_array('HOSTNAME', {node.name: node.hostname
		    for node in self.nodes}) + \
	    Resource.m4_define_array('ADDRESS', {node.name: node.addr + ':' + str(node.port)
		    for node in self.nodes})
	...
	fd = os.open(os.path.join(os.environ['DRBD_LOG_DIR'], posfile), os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0666)
	os.write(fd, data)
	os.close(fd)
	...
	self.peer_devices.event(r'peer-device .* peer-disk:UpToDate', timeout=300)
...
class Node(exxe.Exxe):
	...
	super(Node, self).__init__(['ssh', '-l', 'root', name, 'exxe', '--syslog'], prefix='%s: ' % name)
	self.resource = resource
	self.name = name
	self.addr = addr if addr else subprocess.check_output(['gethostip', '-d', name]).strip()
	self.port = port
	...
	try:
	    ...
	    raise RuntimeError('%s: Directory %s does not exist' % (self.name, DRBD_TEST_DATA))

	self.run(['timeout', os.environ['EXXE_TIMEOUT']], prepare=True)
	self.run(['export', 'PATH=%s:$PATH' % DRBD_TEST_DATA], quote=False, prepare=True)
	self.run(['export', 'DRBD_TEST_DATA=%s' % DRBD_TEST_DATA,
			    ...
			    'DRBD_TEST_VERBOSE=%s' % verbosity_level], prepare=True)
	self.hostname = self.run(['hostname', '-f'], return_stdout=True, prepare=True)
	drbd_version = self.run(['drbd-version'], return_stdout=True, prepare=True)
	...
	return subprocess.check_output(['m4', '-P', '-I', os.path.join(TOP, 'lib', 'm4'), \
	...
	if self.config_changed:
	    self.config_changed = False
	    config = self.config()
	    file = open(os.path.join(self.resource.logdir, 'drbd.conf-%s' % self.name), 'w')
	    file.write(config)
	    file.close
	    self.run(['install-config'], stdin=StringIO(config), prepare=True)

    def listen_to_events(self):
	file = open(os.path.join(self.resource.logdir, 'events-%s' % self.name), 'w')
    ...
    def after_up(self):
	""" When a node is brought up, it normally connects to all the other nodes. """
...
def setup(parser=argparse.ArgumentParser(),
    ...
    Keyword arguments:
    parser -- command line argument parser to use (for recognizing additional arguments)
    nodes, min_nodes, max_nodes -- exact, minimum, and maximum number of test nodes required
    """
    ...
    resource = Resource(args.resource, logdir=args.logdir, template=args.template)
	...
	for node in args.node:
	    ...
	    subprocess.check_call(['screen', '-S', logfile, '-p',
				   '0', '-X', 'logfile',
				   os.path.join(args.logdir, 'console-%s.log' % node)])
	    subprocess.check_call(['screen', '-S', logfile, '-p',
				   '0', '-X', 'log', 'on'])
